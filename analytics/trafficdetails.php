	...
	function getRecords($params) {
	 ...
	 if( !empty($params['searchPhrase']) ) { 		
		...
		$status='<b style="color:#ff6d6d">'.$h.'</b>';
			...
			$data[] =array ("number"=>++$id, "id"=>$row['id'],"token"=>$row['token'],"refer_url"=>$row['refer_url'],"webpage"=>$row['webpage'],"ip_address"=>$row['ip_address'],"country_code"=>$row['country_code'],"country_name"=>$row['country_name'],"region_name"=>$row['region_name'],"city"=>$row['city'],"device"=>$row['device'],"device_type"=>$row['device_type'],"time"=>$row['time'],"again"=>$row['again'],"status"=>$status);
		}
		$json_data = array(
