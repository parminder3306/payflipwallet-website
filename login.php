...
?>
    ...
    <head>
        ...
        <meta name="description" content="Login - PayflipWallet ,online recharge fast service in india, prepaid recharges, postpaid recharges, dth recharges, googleplay recharge and more..">
        ...
        <script>
            $('document').ready(function() {
                        ...
                        function submitForm() {
                            ...
                            $.ajax({
                                    type: 'POST',
                                    url: 'https://payflipwallet.com/loginproccess.php',
                                    data: data,
                                    dataType: "json",
									crossDomain: true,
                                    success: function(status) {
                                        if (status.login == "success") {
                                            setTimeout('window.location = "<?php echo (isset($_SESSION['page']))?$sp->real_escape_string(trim($_SESSION['page'])):'https://payflipwallet.com/';?>";',2000);
                                                $("#login_button").html('<span class="spinner"></span>')
                                            } else if (status.login == "lock") {
                                                msghide();
                                                $("#error").html(status.msg);
                                            } else if (status.login == "account") {
                                                msghide();
                                                $("#error").html(status.msg);
                                            } else if (status.login == "error") {
                                                msghide();
                                                $("#error").html(status.msg);
                                            } else if (status.login == "wrong") {
                                                msghide();
                                                $("#error").html(status.msg);
                                            }
                                        }
...
<div class="headerlogo">PayflipWallet</div>
            <div class="header-right">
                <div class="wallet_icon">
                    <div class="header-right-wallet">
                        <a href="payflipwallet" title="Payflip Wallet">
                            <div class="top"><span class="header-round-icon">&#x20B9;0/-</span></div>
                    </div>
                </div>
                ...
                <div class="user_icon">
                    <div class="header-right-user">
                        <div style="font-size:11px"><a href="https://payflipwallet.com/login">LOG IN</a> | <a href="https://payflipwallet.com/signup">REGISTER</a></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="recharge_card" style="margin-bottom:35px">
            <div class="card-header">Login Account</div>
            <br>
            <div id="error"></div>
            <form method="POST" action="" autocomplete="off" id="login-form">
                <img style="margin:0 0 -40px" src="resource/images/ic_mail.png" height="20" width="20">
                <input type="text" name="user" id="user" placeholder="Mobile Number or Email">
                ...
                <div class="forget"><a href="https://payflipwallet.com/forget">Forgot Password ?</a>
                    <br>
                </div>
                <button type="submit" name="login_button" id="login_button" class="btn">Login</button>
                <div style="padding-top:10px;text-align:center">New User? <a href="https://payflipwallet.com/signup" style="color:#fa8072" title="Create a New Account">Click Here</a></div>
            </form>
            <br>
        </div>
